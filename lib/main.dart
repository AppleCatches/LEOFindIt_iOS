...
class _LeoTrackerAppState extends State<LeoTrackerApp>
  ...
  // Sorting Validity Lock Array to stop jumping lists
  ...
  void _startScanTimer() {
    ...
    _scanTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      setState(() {
        ...
        // Garbage collect older duplicates/stale devices that have rotated/been gone for 10 minutes
        final nowMs = DateTime.now().millisecondsSinceEpoch;
        _devicesBySig.removeWhere((sig, d) => nowMs - d.lastSeenMs > 10 * 60 * 1000);
      });
    });
  }
  ...
  void initState() {
    ...
    _bleSub = BleBridge.detections.listen((device) async {
      ...
      setState(() {
        ...
        _heldPeakUntilMs[device.signature] = nowMs + 10000;
      });
    });
  ...
  // Uses Dynamic sorting with a 1.5 second lock to keep devices from jumping
  List<TrackerDevice> get devices {
    final now = DateTime.now();
    final nowMs = now.millisecondsSinceEpoch;
    
    final currentSigs = _devicesBySig.keys.toSet();
    final displaySigs = _displayOrder.toSet();
    
    // Sort instantly if a brand new tag is discovered, otherwise wait 1.5 seconds to refresh the order
    bool newDeviceAdded = !displaySigs.containsAll(currentSigs);
    bool needsSort = newDeviceAdded || now.difference(_lastSortTime).inMilliseconds >= 1500;

    for (final sig in _heldPeakUntilMs.keys.toList()) {
      if ((_heldPeakUntilMs[sig] ?? 0) < nowMs) {
        _heldPeakRssi.remove(sig);
        _heldPeakUntilMs.remove(sig);
      }
    }
    
    // Append any newly discovered devices to the bottom of the list immediately
    if (newDeviceAdded) {
      final missing = currentSigs.difference(displaySigs);
      _displayOrder.addAll(missing);
    }

    if (needsSort) {
      final list = _displayOrder
          .where((s) => _devicesBySig.containsKey(s))
          .map((s) => _devicesBySig[s]!)
          .toList();

      if (FiltersModel.state.sortMode == SortMode.distanceAsc) {
        list.sort((a, b) => a.smoothedDistanceMeters.compareTo(b.smoothedDistanceMeters));
      } else {
        list.sort((a, b) {
    ...
    return _displayOrder
        .where((sig) => _devicesBySig.containsKey(sig))
  ...
  Widget build(BuildContext context) {
    // Hide generic apple and samsung devices unless they strictly match specific classification profiles
    final trackedDevices = devices
        ...
        .where((d) => d.lastSeenMs >= _mainListClearTime.millisecondsSinceEpoch)
        .toList();
